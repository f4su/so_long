...
void display_log(GLuint object, void (*param_func)(), void (*getlog_func)())
{
  ...
  param_func(object, 0x8B84, &log_length);
  log = malloc(log_length);
  getlog_func(object, log_length, ((void *)0), log);
  fprintf(__stderrp, "%s", log);
  free(log);
}
...
int mlx_shaders_pixel(glsl_info_t *glsl)
{
  ...
  glsl->pixel_vshader = glCreateShader(0x8B31);
  ...
  glGetShaderiv(glsl->pixel_vshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile pixel vshader :\n");
    display_log(glsl->pixel_vshader, glGetShaderiv, glGetShaderInfoLog);
    return (1);
  }

  glsl->pixel_fshader = glCreateShader(0x8B30);
  source = strdup("#version 110 \n"
  ...
  glGetShaderiv(glsl->pixel_fshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile pixel fshader :\n");
  ...
  glGetProgramiv(glsl->pixel_program, 0x8B82, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to link pixel shader program:\n");
...
int mlx_shaders_image(glsl_info_t *glsl)
{
  ...
  glsl->image_vshader = glCreateShader(0x8B31);
  source = strdup("#version 110 \n"
    "attribute vec2 position;"
  ...
  glGetShaderiv(glsl->image_vshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile image vshader :\n");
    display_log(glsl->image_vshader, glGetShaderiv, glGetShaderInfoLog);
    return (1);
  }

  glsl->image_fshader = glCreateShader(0x8B30);
  source = strdup("#version 110 \n"
    "uniform sampler2D texture;"
    "varying vec2 texcoord;"
    "void main()"
  ...
  glGetShaderiv(glsl->image_fshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile image fshader :\n");
  ...
  glGetProgramiv(glsl->image_program, 0x8B82, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to link image shader program:\n");
...
int mlx_shaders_font(glsl_info_t *glsl)
{
  ...
  glsl->font_vshader = glCreateShader(0x8B31);
  source = strdup("#version 110 \n"
    "attribute vec2 position;"
    "uniform vec2 winhalfsize;"
  ...
  glGetShaderiv(glsl->font_vshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile font vshader :\n");
    display_log(glsl->font_vshader, glGetShaderiv, glGetShaderInfoLog);
    return (1);
  }

  glsl->font_fshader = glCreateShader(0x8B30);
  source = strdup("#version 110 \n"
    "uniform sampler2D texture;"
    "uniform vec4 color;"
    "varying vec2 texcoord;"
  ...
  glGetShaderiv(glsl->font_fshader, 0x8B81, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to compile font fshader :\n");
  ...
  glGetProgramiv(glsl->font_program, 0x8B82, &action_ok);
  if (!action_ok) {
    fprintf(__stderrp, "Failed to link font shader program:\n");
